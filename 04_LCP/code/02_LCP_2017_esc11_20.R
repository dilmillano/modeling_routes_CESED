#############################################################################################
# SCRIPT: Rutas LCP 2017 - Escenarios 11 a 20 (conductancia compuesta)
# AUTORA: Diana Millan
#
# Este script asume que:
#  - Ya corriste el script de escenarios 1-10
#  - Existen (o se crearán) los archivos:
#      * Frecuencia_Origen_Destino_2017.csv
#      * Rutas_Origen_Destino_2017.csv
#############################################################################################
...
###########################
# 2. Orígenes, destinos y asignaciones (si ya existen, se reutilizan)
###########################

if (file.exists(path_freq_csv) && file.exists(path_rutas_csv)) {
  cat("Encontrados CSV existentes de rutas/frecuencias. Se reutilizarán.\n")
  
  rutas_directas    <- read.csv(path_rutas_csv)
  frecuencia_directa <- read.csv(path_freq_csv)
  
} else {
  cat("No se encontraron CSV de rutas: se recalcularán asignaciones y tablas.\n")
...
# Asegurarnos de tener rutas_directas cargado
if (!exists("rutas_directas")) {
...
# Valores base del ESCENARIO 1 (los mismos que usaste antes)
base_vals <- c(
  "0" = 950,  # fondo
  "1" = 10,   # ríos
  "2" = 40,   # vías primarias
...
# Ahora queremos escenarios 11 a 20
escenarios <- 11:20

# Nueva semilla para que no repita los aleatorios de 1-10
set.seed(456)

###########################
# 6. Loop sobre escenarios 11-20
###########################
...
cat("\n?????? Escenarios 11-20 procesados y guardados en:\n", escenarios_dir, "\n")
